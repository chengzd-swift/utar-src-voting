    ...
    try {
      ...
      for (let i = 0; i < pending.length; i += chunkSize) {
        ...
        try {
          ...
          const msg = (err.reason || err.shortMessage || err.message || "Transaction failed").slice(0, 250);
          await db.query(
      ...
      console.error(err);
      res.status(500).json({ error: err.reason || err.message || "Batch registration failed" });
    }
  });
